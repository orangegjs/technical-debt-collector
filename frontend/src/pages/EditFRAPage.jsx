// BCE Boundary: :UpdateFRAPage / :SuspendFRAPage / :RetrieveFRAPage
// Methods: validateEnteredData(), displayInputErrorMessage(), displayUpdateSuccess(), displaySuspendSuccess(), displaySuspendFail()

import React, { useEffect, useState } from 'react'
import { useNavigate, useParams } from 'react-router-dom'
import Sidebar from '../components/Sidebar'
import SuspendConfirmModal from '../components/SuspendConfirmModal'
import { retrieveFRA, updateFRA, suspendFRA } from '../api/fraActivityApi'
import { listFRACategories } from '../api/fraCategoryApi'
import { searchUserAcc } from '../api/userAccountApi'

const STATUSES = ['Active', 'Inactive', 'Suspended']

export default function EditFRAPage() {
  const { id } = useParams()
  const navigate = useNavigate()

  const [fraID, setFraID] = useState(null)
  ...
  useEffect(() => {
    async function loadFRA() {
      try {
        // displayFRA
        const fra = await retrieveFRA(Number(id))
        setFraID(fra.fraID)
        setOriginalStatus(fra.fraStatus || 'Active')
        setForm({
          fraName: fra.fraName || '',
          fraDescription: fra.fraDescription || '',
          fraGoalAmount: fra.fraGoalAmount ?? '',
          fraStartDate: fra.fraStartDate || '',
          fraEndDate: fra.fraEndDate || '',
          fraStatus: fra.fraStatus || 'Active',
          fraCategoryID: fra.fraCategoryID ? String(fra.fraCategoryID) : '',
          fraOwnerID: fra.fraOwnerID ? String(fra.fraOwnerID) : '',
        })
      } catch {
        setGlobalError('Failed to load FRA.')
      } finally {
        setLoading(false)
      }
    }
  ...
  async function doSuspend() {
    setSaving(true)
    try {
      await suspendFRA(Number(id))
      // displaySuspendSuccess
      navigate('/fra-management')
    } catch {
      // displaySuspendFail
      setGlobalError('Failed to suspend FRA. Please try again.')
  ...
  async function doUpdate() {
    ...
    try {
      ...
      setGlobalError('Failed to update FRA. Please check your inputs.')
      setShowConfirm(false)
    } finally {
      setSaving(false)
    }
  ...
  async function handleConfirm() {
    ...
    if (form.fraStatus === 'Suspended' && originalStatus !== 'Suspended') {
      setShowConfirm(true)
      return
    }
    ...
    <div className="flex min-h-screen bg-lightbg">
      ...
      <main className="flex-1 flex flex-col">
        <header className="bg-headerbg py-5 text-center">
          <h1 className="text-2xl font-bold text-navy">Edit FRA</h1>
        </header>
      ...
      {showConfirm && (
        <SuspendConfirmModal
          ...
          loading={saving}
          title="Suspend this FRA?"
          message="Are you sure you want to suspend this fundraising activity?"
        />
      )}
    </div>
  )
