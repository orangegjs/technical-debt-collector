// BCE Boundary: :UpdateFRACategoryPage / :SuspendFRACategoryPage / :RetrieveFRACategoryPage
// Methods: displayFRACategory(), validateEnteredData(), displayInputErrorMessage(),
//          displayUpdateSuccess(), displayConfirmationMessage(),
//          displaySuspendSuccess(), displaySuspendFail()

import React, { useState, useEffect } from 'react'
import { useParams, useNavigate } from 'react-router-dom'
import Sidebar from '../components/Sidebar'
import SuspendConfirmModal from '../components/SuspendConfirmModal'
import { retrieveCategory, updateCategory, suspendCategory } from '../api/fraCategoryApi'

const STATUSES = ['Active', 'Inactive']

export default function EditFRACategoryPage() {
  const { id } = useParams()
  const navigate = useNavigate()

  const [form, setForm] = useState({
    categoryName: '',
    categoryDescription: '',
    categoryStatus: 'Active',
  })
  const [categoryID, setCategoryID] = useState(null)
  const [originalStatus, setOriginalStatus] = useState('Active')
  const [errors, setErrors] = useState({})
  const [globalError, setGlobalError] = useState('')
  const [loading, setLoading] = useState(true)
  const [saving, setSaving] = useState(false)
  const [showConfirm, setShowConfirm] = useState(false)

  useEffect(() => {
    async function load() {
      try {
        // displayFRACategory()
        const category = await retrieveCategory(Number(id))
        setCategoryID(category.categoryID)
        setOriginalStatus(category.categoryStatus || 'Active')
        setForm({
          categoryName: category.categoryName || '',
          categoryDescription: category.categoryDescription || '',
          categoryStatus: category.categoryStatus || 'Active',
        })
      } catch {
        setGlobalError('Failed to load category.')
      } finally {
        setLoading(false)
      }
    }
    load()
  }, [id])

  function handleChange(field, value) {
    setForm((prev) => ({ ...prev, [field]: value }))
    setErrors((prev) => ({ ...prev, [field]: '' }))
    setGlobalError('')
  }

  function validateEnteredData() {
    const errs = {}
    if (!form.categoryName.trim()) errs.categoryName = 'Category name is required.'
    return errs
  }

  async function doSuspend() {
    setSaving(true)
    try {
      await suspendCategory(Number(id))
      // displaySuspendSuccess
      navigate('/fra-category-management')
    } catch {
      // displaySuspendFail
      setGlobalError('Failed to suspend category. Please try again.')
      setShowConfirm(false)
    } finally {
      setSaving(false)
    }
  }

  async function doUpdate() {
    setSaving(true)
    try {
      await updateCategory(Number(id), {
        categoryName: form.categoryName.trim(),
        categoryDescription: form.categoryDescription.trim() || null,
        categoryStatus: form.categoryStatus,
      })
      // displayUpdateSuccess
      navigate('/fra-category-management')
    } catch {
      // displayInputErrorMessage
      setGlobalError('Failed to update category. Please check your inputs.')
      setShowConfirm(false)
    } finally {
      setSaving(false)
    }
  }

  async function handleConfirm() {
    const errs = validateEnteredData()
    if (Object.keys(errs).length > 0) {
      // displayInputErrorMessage
      setErrors(errs)
      return
    }
    // displayConfirmationMessage when transitioning to Inactive
    if (form.categoryStatus === 'Inactive' && originalStatus !== 'Inactive') {
      setShowConfirm(true)
      return
    }
    await doUpdate()
  }

  if (loading) {
    return (
      <div className="flex min-h-screen">
        <Sidebar />
        <main className="flex-1 bg-lightbg flex items-center justify-center">
          <div className="w-8 h-8 border-4 border-primary border-t-transparent rounded-full animate-spin" />
        </main>
      </div>
    )
  }

  return (
    <div className="flex min-h-screen bg-lightbg">
      <Sidebar />
      <main className="flex-1 flex flex-col">
        <header className="bg-headerbg py-5 text-center">
          <h1 className="text-2xl font-bold text-navy">Edit FRA Category</h1>
        </header>

        <div className="flex-1 p-8 flex flex-col gap-5">
          <div className="bg-white rounded-2xl p-8 shadow-sm flex flex-col gap-6">
            <div className="grid grid-cols-2 gap-6">
              <Field label="Category ID">
                <input
                  readOnly
                  value={categoryID ?? ''}
                  className="w-full px-4 py-2.5 rounded-xl bg-gray-100 border border-gray-200 text-sm text-gray-500 outline-none cursor-not-allowed"
                />
              </Field>

              <Field label="Category Status">
                <div className="relative">
                  <select
                    value={form.categoryStatus}
                    onChange={(e) => handleChange('categoryStatus', e.target.value)}
                    className="w-full px-4 py-2.5 rounded-xl border border-gray-200 text-sm text-gray-700 outline-none focus:border-primary appearance-none bg-white transition-colors"
                  >
                    {STATUSES.map((s) => (
                      <option key={s} value={s}>{s}</option>
                    ))}
                  </select>
                  <ChevronDown />
                </div>
              </Field>

              <Field label="Category Name" error={errors.categoryName} className="col-span-2">
                <input
                  value={form.categoryName}
                  onChange={(e) => handleChange('categoryName', e.target.value)}
                  className={`w-full px-4 py-2.5 rounded-xl border text-sm text-gray-700 outline-none focus:border-primary transition-colors ${
                    errors.categoryName ? 'border-deletered' : 'border-gray-200'
                  }`}
                />
              </Field>
            </div>

            <Field label="Category Description">
              <textarea
                value={form.categoryDescription}
                onChange={(e) => handleChange('categoryDescription', e.target.value)}
                rows={4}
                placeholder="Describe this fundraising category"
                className="w-full px-4 py-2.5 rounded-xl border border-gray-200 text-sm text-gray-700 outline-none focus:border-primary transition-colors resize-none"
              />
            </Field>

            {globalError && (
              <p className="text-deletered text-sm">{globalError}</p>
            )}

            <div className="flex justify-end gap-3 mt-2">
              <button
                onClick={() => navigate('/fra-category-management')}
                className="px-5 py-2 rounded-lg bg-cancelgray text-white text-sm font-semibold hover:bg-gray-600 transition-colors"
              >
                Cancel
              </button>
              <button
                onClick={handleConfirm}
                disabled={saving}
                className="px-5 py-2 rounded-lg bg-primary text-white text-sm font-semibold hover:bg-blue-700 transition-colors disabled:opacity-60"
              >
                {saving ? 'Saving...' : 'Confirm'}
              </button>
            </div>
          </div>
        </div>
      </main>

      {showConfirm && (
        <SuspendConfirmModal
          onConfirm={doSuspend}
          onCancel={() => setShowConfirm(false)}
          loading={saving}
          title="Set Category to Inactive?"
          message="Are you sure you want to suspend this category? Setting it to Inactive will hide it from active fundraising. You can reactivate it later."
        />
      )}
    </div>
  )
}

function Field({ label, children, error, className }) {
  return (
    <div className={`flex flex-col gap-1.5 ${className || ''}`}>
      <label className="text-sm font-medium text-gray-600">{label}</label>
      {children}
      {error && <p className="text-deletered text-xs">{error}</p>}
    </div>
  )
}

function ChevronDown() {
  return (
    <div className="absolute right-3 top-1/2 -translate-y-1/2 pointer-events-none">
      <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#6b7280" strokeWidth="2" strokeLinecap="round">
        <polyline points="6 9 12 15 18 9" />
      </svg>
    </div>
  )
}
